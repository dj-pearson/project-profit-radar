-- Setup Missing Funnel Steps
-- This script connects the existing email templates to the existing funnels

-- NEWSLETTER FUNNEL STEPS
-- Connect Newsletter templates to Newsletter funnel
...
-- TRIAL FUNNEL STEPS
-- Connect Trial templates to Trial funnel
INSERT INTO public.funnel_steps (
  funnel_id,
  step_order,
  name,
...
-- First create the contact form funnel (since it doesn't exist yet)
...
-- Get the contact form funnel ID (you'll need to run this and update the IDs below)
-- SELECT id FROM public.lead_funnels WHERE trigger_event = 'contact_form' AND company_id = 'fcfd2e31-637b-466b-b533-df70f7f1b3af'::uuid;

-- CONTACT FORM FUNNEL STEPS
-- NOTE: Replace 'CONTACT_FORM_FUNNEL_ID_HERE' with the actual funnel ID from the query above
/*
INSERT INTO public.funnel_steps (
  funnel_id,
  step_order,
  ...
  -- Get contact form template IDs first:
  -- SELECT id, name FROM public.email_templates WHERE category = 'contact_form' ORDER BY name;
  
  -- Step 1: Immediate Response (Immediate)
  ('CONTACT_FORM_FUNNEL_ID_HERE'::uuid, 1, 'Immediate Response', 'IMMEDIATE_RESPONSE_TEMPLATE_ID'::uuid, 0, 'minutes', true),
  
  -- Step 2: Case Study (Day 1)
  ('CONTACT_FORM_FUNNEL_ID_HERE'::uuid, 2, 'Case Study Follow-up', 'CASE_STUDY_TEMPLATE_ID'::uuid, 1, 'days', true),
  
  -- Step 3: Address Concerns (Day 3)
  ('CONTACT_FORM_FUNNEL_ID_HERE'::uuid, 3, 'Address Common Concerns', 'ADDRESS_CONCERNS_TEMPLATE_ID'::uuid, 2, 'days', true),
  
  -- Step 4: Social Proof (Day 5)
  ('CONTACT_FORM_FUNNEL_ID_HERE'::uuid, 4, 'Social Proof & Urgency', 'SOCIAL_PROOF_TEMPLATE_ID'::uuid, 2, 'days', true),
  
  -- Step 5: Final Follow-up (Day 7)
  ('CONTACT_FORM_FUNNEL_ID_HERE'::uuid, 5, 'Final Follow-up', 'FINAL_FOLLOWUP_TEMPLATE_ID'::uuid, 2, 'days', true);
*/

-- UPDATE FUNNEL TOTALS
-- Update the total_steps count for each funnel
...
-- Check Contact Form Templates are available
