...
const EDGE = {
  ...
  AMAZON_SMTP_USER_NAME: ['required', S, 'SES over SMTP, the fallback transport of _shared/ses-email-service.ts (all transactional mail since US-253). With neither these nor AWS_SES_ACCESS_KEY_ID set, every send fails and is logged to email_deliveries and Sentry.'],
  ...
  AWS_SES_ACCESS_KEY_ID: ['optional', S, 'Mail goes over SES SMTP instead of the SES v2 API, so no SES MessageId is recorded in email_deliveries and bounces cannot be matched by id. IAM user limited to ses:SendEmail (US-253).'],
  AWS_SES_SECRET_ACCESS_KEY: ['optional', S, 'Same as AWS_SES_ACCESS_KEY_ID.'],
  AWS_SES_REGION: ['optional', S, 'Taken from AMAZON_SMTP_ENDPOINT, else us-east-1. Must be the region the brikly.net identity is verified in.'],
  AWS_SES_CONFIGURATION_SET: ['optional', S, 'No SES configuration set is named on API sends, so bounce/complaint events are not published anywhere.'],
  SENDGRID_API_KEY: ['optional', S, 'Read only inside a commented-out example in send-scheduled-emails, which does not send yet. Nothing sends through SendGrid (US-253).'],
