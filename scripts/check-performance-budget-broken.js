  ...
  function analyzeDirectory(dirPath, prefix = '') {
    ...
    files.forEach(file => {
      ...
      if (stat.isDirectory()) {
        ...
        // Skip analysis files that shouldn't count toward bundle budget
        if (file === 'stats.html' || file.endsWith('.map') || file.endsWith('-report.html')) {
          console.log(`  ⏭️  Skipping: ${relativePath} (analysis file)`);
          return; // Skip this file, don't include in totals
        }
