...
const DocumentManagement = () => {
  ...
  // as if the folder were empty. The table says the load failed instead.
  const {
    documents,
    ...
    optionsError,
    refetchOptions,
    invalidate: reloadDocuments,
    insert,
    remove,
    move,
    tag,
  } = useDocumentManagement<Document>(projectId);
  ...
  // US-268: react-hook-form + documentUploadFormSchema
  const uploadForm = useForm<DocumentUploadFormValues>({
    resolver: zodResolver(documentUploadFormSchema),
    defaultValues: DOCUMENT_UPLOAD_DEFAULTS,
  });
  const useSmartProcessing = uploadForm.watch('smartProcessing');
  ...
  const handleFileUpload = async (values: DocumentUploadFormValues) => {
    // The schema guarantees at least one file.
    const selectedFiles = values.files as FileList;

    // Check if smart processing is enabled and file is an image/PDF
    const file = selectedFiles[0];
    const isImageOrPDF = file.type.startsWith('image/') || file.type === 'application/pdf';
    
    if (values.smartProcessing && isImageOrPDF && selectedFiles.length === 1) {
  ...
  const performRegularUpload = async (ocrData?: OCRProcessingResult) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        await insert.mutateAsync(documentData);
      ...
      uploadForm.reset(DOCUMENT_UPLOAD_DEFAULTS);
      setUploadProgress(0);
  ...
  const confirmDeleteDocument = async () => {
    ...
    try {
      ...
      await remove.mutateAsync([deleteConfirmDoc.id]);
  ...
  const bulkMoveToFolder = async () => {
    ...
    setBulkLoading(true);
    try {
      await move.mutateAsync({ ids, categoryId: moveToCategory });
  ...
  const bulkTagDocs = async () => {
    const newTag = docTagToAdd.trim();
    const ids = [...selectedDocIds];
    if (!newTag || ids.length === 0) return;
    setBulkLoading(true);
    try {
      await tag.mutateAsync({ docs: filteredDocuments.filter((d) => selectedDocIds.has(d.id)), tag: newTag });
      toast({ title: 'Documents tagged', description: `"${newTag}" added to ${ids.length} document(s).` });
      setDocTagToAdd('');
      setSelectedDocIds(new Set());
    } catch (err: any) {
      ...
      <div className="flex justify-end mb-6 gap-2">
        ...
        <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
          ...
          <DialogContent className="sm:max-w-md" aria-describedby="upload-documents-description">
            ...
            <Form {...uploadForm}>
            <form onSubmit={uploadForm.handleSubmit(handleFileUpload)} noValidate className="space-y-4" aria-label="Upload documents form">
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-2 mt-2">
                  <input
                    ...
                    onChange={(e) => uploadForm.setValue('smartProcessing', e.target.checked)}
                    className="rounded"
                  />
          ...
          <CardContent className="p-4">
            {optionsError && (
              <p role="alert" className="mb-3 text-sm text-destructive">
                Folders and projects could not be loaded, so the filters and the upload folder list may be incomplete.{' '}
                <button type="button" className="underline" onClick={refetchOptions}>Try again</button>
              </p>
            )}
