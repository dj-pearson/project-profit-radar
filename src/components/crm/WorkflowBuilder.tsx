...
export function WorkflowBuilder({ workflowId }: WorkflowBuilderProps) {
  ...
  return (
    <div className="h-screen flex">
      ...
      {/* Main Canvas or Templates or Analytics or Tester or Scheduler or Versions or Error Handling */}
      {showTemplates ? (
        ...
        <div className="flex-1 p-8 overflow-auto">
          <div className="max-w-3xl mx-auto">
            <div className="mb-6">
              <Button variant="outline" onClick={() => setShowErrorHandling(false)}>
                ← Back to Canvas
              </Button>
            </div>
            <WorkflowErrorHandling workflowId={workflowId} />
          </div>
        </div>
      ) : (
      ...
      {!showTemplates && !showAnalytics && !showTester && !showScheduler && !showVersions && !showErrorHandling && selectedNode && selectedNode.id !== "trigger" && (
