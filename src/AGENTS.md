...
- `integrations/supabase/types.ts` - **generated** DB types. Regenerate with `npm run db:types` (needs the Supabase CLI + `SUPABASE_DB_URL` pointing at the self-hosted Postgres; see README); never hand-edit — edits are overwritten on the next generation.
