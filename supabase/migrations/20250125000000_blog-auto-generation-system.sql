-- Blog Auto-Generation System Migration
-- Adds automated blog generation with AI scheduling and enhanced SEO features
...
CREATE TABLE public.blog_auto_generation_settings (
  ...
  generation_frequency TEXT NOT NULL DEFAULT 'weekly', -- 'daily', 'weekly', 'biweekly', 'monthly'
  ...
  -- AI Model settings
  preferred_ai_provider TEXT NOT NULL DEFAULT 'claude', -- 'claude', 'openai', 'gemini'
  ...
  -- SEO settings
  optimize_for_geographic BOOLEAN DEFAULT false, -- Geographic/Local SEO
  target_locations TEXT[] DEFAULT '{}', -- Geographic targets for local SEO
  seo_focus TEXT DEFAULT 'balanced', -- 'traditional', 'geo', 'balanced'
  ...
  minimum_topic_gap_days INTEGER DEFAULT 30, -- Days before repeating similar topics
...
CREATE TABLE public.blog_topic_history (
  ...
  topic_category TEXT,
  content_type TEXT DEFAULT 'article', -- 'article', 'guide', 'case_study', 'news'
...
CREATE TABLE public.blog_generation_queue (
  ...
  -- Metadata
  generation_type TEXT DEFAULT 'scheduled', -- 'scheduled', 'manual', 'triggered'
...
CREATE TABLE public.ai_model_configurations (
  ...
  model_family TEXT, -- 'claude-3', 'gpt-4', 'gemini-pro'
  ...
  cost_rating INTEGER, -- 1-10 (10 = most expensive)
  ...
  requires_api_key TEXT, -- Environment variable name for API key
...
CREATE TABLE public.blog_content_analysis (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_blog_auto_gen_settings_company ON public.blog_auto_generation_settings(company_id);
CREATE INDEX idx_blog_topic_history_company ON public.blog_topic_history(company_id);
CREATE INDEX idx_blog_topic_history_created_at ON public.blog_topic_history(created_at);
CREATE INDEX idx_blog_generation_queue_scheduled_for ON public.blog_generation_queue(scheduled_for);
CREATE INDEX idx_blog_generation_queue_status ON public.blog_generation_queue(status);
CREATE INDEX idx_blog_generation_queue_company ON public.blog_generation_queue(company_id);
CREATE INDEX idx_ai_model_configurations_provider ON public.ai_model_configurations(provider);
CREATE INDEX idx_ai_model_configurations_active ON public.ai_model_configurations(is_active);
CREATE INDEX idx_blog_content_analysis_blog_post ON public.blog_content_analysis(blog_post_id);
...
INSERT INTO public.ai_model_configurations (provider, model_name, model_display_name, model_family, max_tokens, context_window, speed_rating, quality_rating, cost_rating, recommended_for_blog, good_for_seo, good_for_long_form, is_active, is_default, requires_api_key, description) VALUES
-- Claude models
