-- Procurement, Dashboards, Client Portal, Billing & Reporting
-- Migration: 20250202000023
-- Description: Final Phase 5 features
...
CREATE INDEX idx_supplier_catalog_price ON supplier_catalog(unit_price);
...
CREATE INDEX idx_purchase_recommendations_status ON purchase_recommendations(status);
...
CREATE INDEX idx_client_portal_project ON client_portal_access(project_id);
...
CREATE INDEX idx_client_messages_unread ON client_messages(is_read) WHERE is_read = FALSE;
...
CREATE INDEX idx_billing_rules_type ON billing_automation_rules(rule_type);
...
CREATE INDEX idx_report_history_date ON report_history(generated_at DESC);
