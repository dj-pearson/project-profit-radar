-- Step 3: Incident Response System for SOC Compliance
...
-- Create RLS policies for incident_response_team
CREATE POLICY "Company users can view response team" 
...
-- Create RLS policies for incident_response_playbooks
CREATE POLICY "Company users can view playbooks" 
...
-- Create RLS policies for incident_activities
CREATE POLICY "Users can view incident activities" 
...
USING (
  EXISTS (
    ...
    WHERE si.id = incident_activities.incident_id 
    AND p.id = auth.uid()
  ) OR
  EXISTS (
    SELECT 1 FROM public.profiles 
...
CREATE POLICY "Users can create incident activities" 
ON public.incident_activities 
FOR INSERT 
WITH CHECK (
  EXISTS (
...
-- Create RLS policies for incident_metrics
CREATE POLICY "Company users can view incident metrics" 
...
BEGIN
  ...
  -- Calculate metrics
  detection_time := EXTRACT(EPOCH FROM (incident_record.detected_at - incident_record.detected_at)) / 60;
