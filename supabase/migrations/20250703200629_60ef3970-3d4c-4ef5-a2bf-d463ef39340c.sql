...
CREATE POLICY "Admins can manage expense categories" ON public.expense_categories
  FOR ALL USING (
    ...
    get_user_role(auth.uid()) = ANY(ARRAY['admin'::user_role, 'accounting'::user_role, 'root_admin'::user_role])
  );
...
CREATE POLICY "Staff can manage expenses" ON public.expenses
  FOR ALL USING (
    company_id = get_user_company(auth.uid()) AND 
    get_user_role(auth.uid()) = ANY(ARRAY['admin'::user_role, 'accounting'::user_role, 'project_manager'::user_role, 'office_staff'::user_role, 'root_admin'::user_role])
  );
...
CREATE POLICY "Project managers can manage budgets" ON public.project_budgets
  FOR ALL USING (
    EXISTS (
      ...
      AND get_user_role(auth.uid()) = ANY(ARRAY['admin'::user_role, 'project_manager'::user_role, 'accounting'::user_role, 'root_admin'::user_role])
    )
  );
