// Analyze Internal Links Edge Function
...
const InternalLinksSchema = z.object({
  url: auditUrl,
  base_domain: z.string().max(255).optional(),
});
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("[ANALYZE-LINKS] User authenticated", { userId: user.id });
    ...
    const parsed = await validateBody(req, InternalLinksSchema, { name: 'analyze-internal-links' });
    if (!parsed.ok) return parsed.response;
    const { url, base_domain } = parsed.data as z.infer<typeof InternalLinksSchema>;
    if (!url) {
    ...
    if (saveError) {
      console.error(
        '[ANALYZE-INTERNAL-LINKS] Analysis completed but was NOT stored:',
        saveError.message,
      );
    }
    ...
    await captureException(error, { fn: 'analyze-internal-links', req });
