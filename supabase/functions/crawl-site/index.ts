...
serve(async (req) => {
  ...
  try {
    ...
    console.log("[CRAWL-SITE] User authenticated", { userId: user.id });
    ...
    const parsed = await validateBody(req, CrawlSiteSchema, { name: 'crawl-site' });
    if (!parsed.ok) return parsed.response;
    const requestData = parsed.data as z.infer<typeof CrawlSiteSchema>;
    ...
    while (queuedUrls.length > 0 && crawledPages.length < max_pages) {
      ...
      try {
        ...
        const pageData = {  // CRITICAL: Site isolation
          url: current.url,
        ...
        crawledPages.push({  // CRITICAL: Site isolation
          url: current.url,
          status_code: 0,
          has_errors: true,
    ...
    await captureException(error, { fn: 'crawl-site', req });
