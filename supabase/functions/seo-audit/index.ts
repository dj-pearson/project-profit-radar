...
const SEOAuditSchema = z.object({
  url: auditUrl,
  audit_type: z.enum(['full', 'quick', 'technical', 'content']).optional(),
});
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("[SEO-AUDIT] User authenticated", { userId: user.id });
    ...
    // Get request data
    const parsed = await validateBody(req, SEOAuditSchema, { name: 'seo-audit' });
    if (!parsed.ok) return parsed.response;
    const requestData = parsed.data as z.infer<typeof SEOAuditSchema>;
    ...
    await captureException(error, { fn: 'seo-audit', req });
