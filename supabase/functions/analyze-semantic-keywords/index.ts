...
const SemanticKeywordsSchema = z.object({
  url: auditUrl,
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("[ANALYZE-SEMANTIC-KEYWORDS] User authenticated", { userId: user.id });
    ...
    const parsed = await validateBody(req, SemanticKeywordsSchema, { name: 'analyze-semantic-keywords' });
    if (!parsed.ok) return parsed.response;
    const { url, primary_keyword } = parsed.data as z.infer<typeof SemanticKeywordsSchema>;
    if (!url) {
    ...
    const analysisData = {  // CRITICAL: Site isolation
      url,
      page_id: null,
    ...
    if (saveError) {
      console.error(
        '[ANALYZE-SEMANTIC-KEYWORDS] Analysis completed but was NOT stored:',
        saveError.message,
      );
    }
    ...
    await captureException(error, { fn: 'analyze-semantic-keywords', req });
