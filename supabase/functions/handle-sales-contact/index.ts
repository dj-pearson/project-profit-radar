...
serve(async (req) => {
  ...
  try {
    ...
    const blockedIp = await rejectBlockedIp(supabaseClient, req, 'handle-sales-contact', corsHeaders);
    ...
    if (!human.ok) {
      console.warn('[handle-sales-contact] Turnstile check failed:', human.reason);
      return new Response(
