 ...
 *
 * SQL mirrors: a storage policy cannot import TypeScript, so two SQL functions
 * restate a small part of this file - public.tier_storage_limit_gb (the
 * storage column of TIER_LIMITS) and public.company_account_read_only
 * (GRACE_PERIOD_DAYS). `generate-tiers.mjs --check` reads the latest migration
 * that defines each and fails if it disagrees with the values here, so a
 * limit change needs a new migration in the same commit.
 *
