...
serve(async (req) => {
  ...
  // SECURITY (US-238): cross-tenant cron analytics — require an admin JWT or
  // the shared CRON_SECRET before processing all users.
  ...
  try {
    ...
    await captureException(error, { fn: 'generate-churn-predictions', req });
