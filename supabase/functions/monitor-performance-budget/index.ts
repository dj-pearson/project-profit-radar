...
serve(async (req) => {
  ...
  try {
    ...
    const parsed = await validateBody(req, PerformanceBudgetSchema, { name: 'monitor-performance-budget' });
    ...
    await captureException(error, { fn: 'monitor-performance-budget', req });
