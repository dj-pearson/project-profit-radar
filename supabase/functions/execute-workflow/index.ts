...
async function startExecution(supabase: any, workflowId: string, triggerData: Record<string, any>) {
  ...
  // Create the execution record that step processing reads and mutates.
  ...
  await executeSteps(supabase, execution, steps, 0)
...
async function continueExecution(supabase: any, executionId: string) {
  ...
  await executeSteps(supabase, execution, steps, execution.completed_steps ?? 0)

  return execution
}

async function executeSteps(supabase: any, execution: any, steps: WorkflowStep[], startIndex = 0) {
...
async function executeEmailStep(supabase: any, step: WorkflowStep, execution: any) {
  ...
  const { data: queuedEmail, error: queueError } = await supabase
    ...
    .insert({
      ...
      from_email: from_email || 'noreply@brikly.net',
      from_name: from_name || 'Brikly',
      reply_to: reply_to,
...
async function executeDelayStep(_supabase: any, step: WorkflowStep, _execution: any) {
