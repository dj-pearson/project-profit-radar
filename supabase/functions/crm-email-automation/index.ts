...
async function processAutomation(
  ...
  const variables = {
    ...
    unsubscribe_url: `https://brikly.net/unsubscribe?email=${encodeURIComponent(recipientEmail)}`,
  ...
  const { data: queuedEmail, error: queueError } = await supabase
    ...
    .insert({
      ...
      from_email: config.fromEmail || 'noreply@brikly.net',
      from_name: config.fromName || 'Brikly',
...
async function scheduleNextCampaignEmail(
  ...
  await supabase
    ...
    .insert({
      ...
      from_email: (campaign.from_email as string) || 'noreply@brikly.net',
      from_name: (campaign.from_name as string) || 'Brikly',
