...
const ExportSeoReportSchema = z.object({
  report_type: z.string().max(50),
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("[EXPORT-SEO-REPORT] User authenticated", { userId: user.id });
    ...
    const parsed = await validateBody(req, ExportSeoReportSchema, { name: 'export-seo-report' });
    if (!parsed.ok) return parsed.response;
    const { report_type, format = 'json', start_date, end_date, url } = parsed.data;
    ...
    switch (report_type) {
      ...
      default:
        return new Response(JSON.stringify({ success: false, timestamp: new Date().toISOString(), error: 'Invalid report_type' }),
    ...
    // Default JSON format
    return new Response(JSON.stringify({ success: true, timestamp: new Date().toISOString(), ...reportData }), {
